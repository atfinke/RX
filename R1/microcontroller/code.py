...
class Main:
    ...
    def __init__(self, force_debug=False):
        print("__init__")
        internal_led = neopixel.NeoPixel(board.NEOPIXEL, 1)
        internal_led[0] = (255, 0, 0)

        self.buttons = Buttons(DeviceConfig.BUTTON_INPUTS)
        self.buttons_count = len(DeviceConfig.BUTTON_INPUTS)

        time.sleep(1)
        _, states = self.buttons.states()
        self._last_button_states = states
        if states[:1] == [True] or force_debug:
            DebugMode(self.buttons, DeviceConfig.BUTTON_TLC_LED_ORDER)

        self.lights = Lights(DeviceConfig.BUTTON_TLC_LED_ORDER)
        self.lights.set_to_colors_tuple((0, 0, int(DeviceConfig.MAX_LED_VALUE / 4)))
    ...
    def update_leds_for_transition(self):
        ...
        progress = (time.monotonic() - self.transition_start_time) / DeviceConfig.TRANSITION_DURATION
