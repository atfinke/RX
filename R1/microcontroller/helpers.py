...
def white_tuple_array(light_count):
    current = []
    for index in range(light_count):
        current.append((Config.MAX_LED_VALUE, Config.MAX_LED_VALUE, Config.MAX_LED_VALUE))
    return current
...
def hex_to_rgb(hex):
    r = gamma[int(hex[0:2], 16)]
    g = gamma[int(hex[2:4], 16)]
    b = gamma[int(hex[4:6], 16)]
    return (r, g, b)